...
Parley es un CRM self-hosted y gratuito para negocios que venden por WhatsApp:
...
¿Ya tienes tu propio agente? Puedes apagar el de Parley y conectar el tuyo por
la [API de servicio `/api/bot/*`](#-trae-tu-propio-agente): el token de WhatsApp
nunca sale del CRM.

![Bandeja de Parley CRM](docs/screenshots/bandeja.png)
...
tuyo, en tu mismo servidor— apaga el agente de Parley y habilita la API de
...
Parley **consume** un token de la WhatsApp Cloud API — no implementa el
...
3. En Parley: **Configuración → WhatsApp** → pega WABA ID + Phone Number ID +
   token → **Probar conexión** → Guardar.
4. En el panel de Meta (WhatsApp → Configuration → Webhook) pega la **URL del
   webhook** y el **verify token** que Parley te muestra, y suscribe el campo
...
clientes; la instancia de Parley del cliente solo recibe su token. El webhook
...
```text
   Meta (WABA del cliente)
        ...
        ▼
   ┌────────────────────────────┐      ┌─────────────────────────────┐
   │  Instancia Parley          │      │  Backend de TU agencia      │
...
> la app dueña. Por eso Parley también **sincroniza plantillas por la API de
...
WhatsApp es el canal por el que existe Parley y siempre está encendido. Los
...
2. En Parley, **Configuración → Messenger**: elige *Zernio*, pega el
...
3. En Zernio, da de alta ese endpoint con el evento `message.received` y el
   ...
   misma URL; Parley solo ingiere aquí lo de Facebook.
...
2. En Parley, **Configuración → Messenger**: elige *App propia de Meta*, pega
...
como `Messenger`, con el nombre de su perfil, y lo que respondas desde Parley
...
1. **Opt-in**: escribe solo a personas que iniciaron la conversación o
   aceptaron recibir mensajes; Parley respeta la ventana de 24 h y bloquea el
...
4. **Sin spam ni broadcast**: Parley no incluye envíos masivos; úsalo para
...
permitidos (números de prueba de Meta) o el formato es inválido. Parley ya
...
**Olvidé mi contraseña y no puedo entrar** — Parley no manda correos (sería una
